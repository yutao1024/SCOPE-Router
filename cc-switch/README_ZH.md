...
这个分支在本地代理链路里加入了 ACRouter hook，覆盖 Claude Code、Codex、OpenCode、OpenClaw 和 Hermes 兼容流量。ACRouter 对直连 API Key Provider 默认开启，请求发往上游之前，会调用一个便宜的 router 模型，从当前 Provider 可用的模型候选中为 coding-agent 任务选择目标模型。如果 router 调用失败，或 Provider 使用的是不适合直连的 OAuth/托管认证流，CC Switch 会自动回退到原有静态模型映射。
...
不配置 `routerModel` 时，CC Switch 会根据本地价格表从候选模型里选择可推断的最低成本模型作为 router。某个 Provider 不想使用 ACRouter 时，把 `"enabled"` 设为 `false`。
