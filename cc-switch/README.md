...
This fork enables SCOPE-Router by default in the local proxy path for Claude
Code, Codex, OpenCode, OpenClaw, and Hermes-compatible traffic. Before a
request is forwarded upstream, CC Switch calls a local SCOPE-Router service to
select one configured backend model for the current agent request. If
SCOPE-Router is disabled, times out, or returns a non-candidate model, CC
Switch falls back to the existing ACRouter/static model mapping path.
...
SCOPE-Router runs before ACRouter and before static model mapping. The selected
candidate is written to `body["model"]`, after which CC Switch continues its
normal provider mapping, auth, streaming, and fallback behavior.

Set `"enabled": false` to disable SCOPE-Router for a provider. Increase
`timeoutMs` if your router service runs on a remote machine or cold-starts
encoders on the first request.
