...
impl RequestForwarder {
    ...
    async fn forward(
        ...
        // SCOPE-Router and ACRouter run before static model mapping so they can
        // select a real upstream model while preserving all existing
        // adapter/transform logic. SCOPE-Router takes priority when enabled;
        // otherwise ACRouter remains the fallback route-decision hook.
        let scope_router_result = super::scope_router::apply_scope_router(
            &self.router,
            app_type,
            provider,
            body.clone(),
        ...
        .await;
        let (routed_body, routed_by_dynamic_router) = if scope_router_result.routed {
            (scope_router_result.body, true)
        } else {
            let acrouter_result = super::acrouter::apply_acrouter(
                &self.router,
                app_type,
                provider,
                scope_router_result.body,
                adapter,
            )
            .await;
            (acrouter_result.body, acrouter_result.routed)
        };
        ...
        } else if routed_by_dynamic_router {
            routed_body
        } else {
