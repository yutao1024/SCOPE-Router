#!/usr/bin/env python3
"""
LinearR router training and evaluation script
...
Usage:
    python routers/linear/train_and_eval.py \
        ...
        --use_soft_labels \
        --train_lambda 10.0 \
        --output_dir outputs/linear_router
"""
...
def main():
    parser = argparse.ArgumentParser(description="Train and evaluate LinearR router")
    ...
    parser.add_argument("--output_dir", default="outputs/linear_router", help="Output directory")
    ...
    print("="*80)
    print("🚀 LinearR router training and evaluation")
    print("="*80)
    print(f"Fusion method: {args.fusion_method}")
    print(f"Regularization: {args.penalty}, C={args.C}")
    if is_hard_label:
    ...
    # Compute average model costs (for cost penalties at inference time)
    ...
    # 3. Train LinearRouter
    print("\n🎓 Training LinearR router...")
    if args.enable_monitoring:
    ...
    model_name = f"linear_{args.fusion_method}"
    if args.penalty == 'l1':
        model_name += "_l1"
    if is_hard_label:
