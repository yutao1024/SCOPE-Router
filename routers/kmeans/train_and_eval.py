...
Usage:
    python routers/kmeans/train_and_eval.py \
        --dataset_dir . \
        --fusion_method normalize_concat \
        --output_dir outputs/kmeans_router
"""
...
from routers.kmeans.router import KMeansRouter
...
def main():
    parser = argparse.ArgumentParser(description="training and evaluation Kmeans ")
    ...
    print("="*80)
    print("🚀 Kmeans training and evaluation")
    print("="*80)
    print(f"Fusion method: {args.fusion_method}")
    print(f"L2: {args.normalize}")
    print(f"Lambda Cost: {args.lambda_cost}")
    print("Label mode: softλ0（uniform over correct； Y[:,k]=1  prototype）")
    ...
    model_name = f"kmeans_{args.fusion_method}"
    if not args.normalize:
        model_name += "_nonorm"
    if args.lambda_cost > 0:
        model_name += f"_l{args.lambda_cost}"

    model_path = output_dir / f"{model_name}.pkl"
    router.save(str(model_path))
    print(f"\n✓ Model saved: {model_path}")
    
    # 5.  (--- ： Rank Score  train/dev/test ---)
    print("\n📊 evaluation...")
    results = {}
